...
function drawLineGraph(){
	...
	$.ajax({
		...
		success:function(data){
			...
			var lineChartData = {
					labels : [tempArr[0].date,tempArr[1].date,tempArr[2].date,tempArr[3].date,tempArr[4].date,tempArr[5].date,tempArr[6].date],
					datasets : [
						{
							...
							data : [tempArr[0].temp,tempArr[1].temp,tempArr[2].temp,tempArr[3].temp,tempArr[4].temp,tempArr[5].temp,tempArr[6].temp]
						}
					]
			...
			var myLine = new Chart(document.getElementById("canvas").getContext("2d")).Line(lineChartData,barOption);
...
function getWeatherCode(){
	...
	$.ajax({
		...
		success:function(data){
			...
			var nowTime = now.getFullYear() + "년" + (now.getMonth()+1) + "월" + now.getDate() + "일" + (now.getHours()+9) + ":" + now.getMinutes()+"분";
			alert("time-"+nowTime);
